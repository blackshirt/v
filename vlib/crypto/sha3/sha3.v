...
fn (mut d Digest) checksum_internal() []u8 {
	...
	/*
	mut result := []u8{cap: d.output_len}
	mut remaining := d.output_len

	for remaining > 0 {
		mut take := int_min(remaining, d.rate)
		result << d.s.to_bytes()[..take]
	...
	return result
	*/
	return d.read_internal(d.output_len)
}
...
// do_pad adds domain separation bits and performs required padding rule into state buffer
fn (mut d Digest) do_pad() {
	// pad the last input bytes to have rate bytes
	if d.input_buffer.len == d.rate - 1 {
		// a single byte pad needs to be handled specially
		// pad the buffer with domain separation suffix and final bit
		d.input_buffer << d.suffix | u8(0x80)
	} else {
		...
		// add intermediate zero pad bytes, ie, multi-rate padding rule
		for _ in 0 .. zero_pads {
			d.input_buffer << u8(0x00)
		}

		// add the last pad final byte
		d.input_buffer << u8(0x80)
	}
}

fn (mut d Digest) absorb(data []u8) {
...
fn (mut d Digest) finalize() {
	...
	d.do_pad()
	// mut padded := d.input_buffer.clone()
	d.s.xor_bytes(d.input_buffer[..d.rate], d.rate)
	d.s.kaccak_p_1600_24()
...
fn (mut d Digest) read_internal(out_len int) []u8 {
